...
export default function SubmitReport() {
  ...
  const [nearbyHazards, setNearbyHazards] = useState<Incident[]>([])
  ...
  useEffect(() => {
    ...
    getIncidents()
      .then((incidents) => {
        ...
        setNearbyHazards(critical.slice(0, 3))
      })
      .catch(() => {
        if (!cancelled) setNearbyHazards([])
      })
      .finally(() => {
  ...
  return (
    <div className="min-h-screen flex flex-col">
      <div className="flex-1 px-6 md:px-8 py-8 max-w-7xl mx-auto w-full">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-7">
            <form onSubmit={handleSubmit} noValidate className="liquid-glass p-6 md:p-8 rounded-[2rem] ghost-border shadow-2xl space-y-8">
              ...
              <div className="space-y-3">
                ...
                {photoPreview ? (
                  <div className="relative rounded-2xl overflow-hidden ghost-border group cursor-zoom-in">
                    <img src={photoPreview} alt="Preview" className="w-full max-h-64 object-cover group-hover:scale-105 transition-all duration-500" />
          ...
          <div className="lg:col-span-5 space-y-8">
            ...
            <div className="liquid-glass rounded-[2rem] ghost-border p-6 relative overflow-hidden">
              ...
              <div className="flex items-center gap-3 mb-5">
                ...
                <h4 className="font-headline text-lg font-bold text-on-surface">Active Hazards Nearby</h4>
              </div>
              ...
              {hazardsLoading ? (
                ...
                <p className="text-sm text-slate-500 py-4 text-center">No active critical or high-severity hazards.</p>
              ) : (
                <div className="space-y-3">
                  {nearbyHazards.map((hazard) => (
                    <Link
                      ...
                      className="flex items-stretch gap-0 rounded-xl overflow-hidden bg-surface-container-highest/30 hover:bg-surface-container-highest/50 transition-colors group"
                    >
                      <div
                        className={`w-1.5 shrink-0 ${
                          hazard.severity === 'critical' ? 'bg-red-500' : 'bg-orange-500'
                        }`}
                      />
                      <div className="flex-1 p-3 pl-4">
                        ...
                        <div className="flex items-center gap-2 mt-1">
                          ...
                          <span className="text-[10px] font-bold uppercase tracking-widest text-slate-500">
                            {hazard.severity} &middot; {hazard.location}
                          </span>
                        </div>
                      </div>
                    </Link>
            ...
            {/* Protocol Adherence */}
            <div className="liquid-glass rounded-[2rem] ghost-border p-6 flex items-start gap-5 relative overflow-hidden">
              <div className="absolute top-0 right-0 p-4 opacity-10">
              ...
              <div>
                <h4 className="font-headline text-lg font-bold text-on-surface mb-1">Protocol Adherence</h4>
                <p className="text-sm text-on-surface-variant leading-relaxed">
                  All submissions are logged, timestamped, and tracked through our secure incident management pipeline. Reports are handled in compliance with organizational safety protocols.
                </p>
              </div>
            </div>
      ...
      {/* Footer */}
      <footer className="mt-16 bg-slate-900/80 backdrop-blur-xl border-t border-outline-variant/10">
        <div className="max-w-7xl mx-auto px-6 md:px-8 py-10">
          <div className="flex flex-col md:flex-row items-center justify-between gap-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm font-bold text-blue-200 font-headline">Hazard Reporter</p>
                <p className="text-[10px] text-slate-500 uppercase tracking-widest">Incident Management System</p>
              </div>
            </div>
            <div className="flex items-center gap-6 text-xs text-slate-500">
              <Link to="/dashboard" className="hover:text-slate-300 transition-colors">Dashboard</Link>
              <Link to="/track" className="hover:text-slate-300 transition-colors">Track Report</Link>
              <a href="#" className="hover:text-slate-300 transition-colors">Privacy Policy</a>
              <a href="#" className="hover:text-slate-300 transition-colors">Terms of Service</a>
            </div>
            <p className="text-[10px] text-slate-600 uppercase tracking-widest">
              &copy; {new Date().getFullYear()} Hazard Reporter. All rights reserved.
            </p>
          </div>
        </div>
      </footer>
